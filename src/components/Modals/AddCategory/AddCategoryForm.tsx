...
export function AddCategoryForm({ onSuccess }: AddCategoryFormProps) {
  ...
  const { handleSubmit, watch, control, reset, formState: { errors } } = useForm<CategoryFormValues>({
  ...
  const onSubmit = (data: CategoryFormValues) => {
    addCategory({ name: data.name, icon: data.icon, color: data.color })
  }
      ...
      <FieldGroup className="mt-2 space-y-6">
        <Field>
          <div className="flex justify-between items-center mb-1">
            <FieldLabel htmlFor="category-name" className="block text-xs text-muted-foreground mb-0">
              CATEGORY NAME
            </FieldLabel>
        ...
        <Field>
          <FieldLabel className="mb-3 block text-xs text-muted-foreground">Select Icon</FieldLabel>
          <Controller
            control={control}
            name="icon"
            render={({ field }) => (
        ...
        <Field>
          <FieldLabel className="mb-3 block text-xs text-muted-foreground">Select Color</FieldLabel>
          <Controller
            control={control}
            name="color"
            render={({ field }) => (

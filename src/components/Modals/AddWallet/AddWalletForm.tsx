...
interface AddWalletFormProps {
  /** Called after a successful wallet creation */
  onSuccess?: () => void
  /** If true, form resets when isOpen changes to false (for modal context) */
  isOpen?: boolean
}

export function AddWalletForm({ onSuccess, isOpen }: AddWalletFormProps) {
  const queryClient = useQueryClient()

  const { handleSubmit, control, watch, reset, formState: { errors } } = useForm<WalletFormValues>({
  ...
  // Reset form when modal closes (only in controlled/modal context)
  useEffect(() => {
    if (isOpen === false) {
      reset()
    }
      ...
      <FieldGroup className="mt-4 space-y-5">
        ...
        <Field>
          <div className="flex justify-between items-center mb-1">
            <FieldLabel htmlFor="wallet-name" className="block text-xs text-muted-foreground mb-0">
              ACCOUNT NAME
            </FieldLabel>
            <span className="text-xs text-muted-foreground font-medium">{currentName.length}/30</span>
          </div>
          <Controller
            control={control}
            name="name"
        ...
        <div className="grid grid-cols-2 gap-4">
          {/* Wallet Type */}
          <Field>
            <FieldLabel className="block text-xs text-muted-foreground">ACCOUNT TYPE</FieldLabel>
            <Controller
              control={control}
              name="type"
              render={({ field }) => (
          ...
          {/* Currency */}
          <Field>
            <FieldLabel className="block text-xs text-muted-foreground">CURRENCY</FieldLabel>
            <Controller
              control={control}
              name="currency"
              render={({ field }) => (
        ...
        {/* Initial Balance */}
        <Field>
          <FieldLabel htmlFor="wallet-balance" className="block text-xs text-muted-foreground">
